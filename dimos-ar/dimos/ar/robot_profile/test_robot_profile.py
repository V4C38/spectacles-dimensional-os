...
def test_g1_handshake_two_70mm_tags() -> None:
    ...
    assert abs(hs.tag_tracking_profile["tag_total_size_m"] - 0.070) < 1e-9
    assert len(G1_DEFAULT_TAG_MOUNTS) == 2
    assert abs(G1_DEFAULT_TAG_MOUNTS[0].size_m - 0.056) < 1e-9
    assert G1_DEFAULT_TAG_MOUNTS[0].position == (0.072, 0.0, 0.178)
    assert G1_DEFAULT_TAG_MOUNTS[1].position == (-0.070, 0.0, 0.237)


def test_g1_chest_tag_faces_forward_and_back_tag_faces_rear() -> None:
    """Third column of T_base_tag is the tag outward normal."""
    from dimos.ar.robot_profile.g1 import G1_DEFAULT_TAG_MOUNTS

    chest, back = G1_DEFAULT_TAG_MOUNTS
    assert np.allclose(chest.T_base_tag[:3, 2], [1.0, 0.0, 0.0])
    assert np.allclose(back.T_base_tag[:3, 2], [-1.0, 0.0, 0.0])
    # Both upright: tag "up" axis is world up.
    assert np.allclose(chest.T_base_tag[:3, 1], [0.0, 0.0, 1.0])
    assert np.allclose(back.T_base_tag[:3, 1], [0.0, 0.0, 1.0])
