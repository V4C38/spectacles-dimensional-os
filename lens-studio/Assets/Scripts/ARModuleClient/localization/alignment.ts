import type { LocalizationResult, Quat, Vec3 } from "../websocket/types";

export interface TOdomClient {
  position: Vec3;
  orientation: Quat;
  confidence: number;
  ts: number;
...
export class Alignment {
  private current: TOdomClient | null = null;

  apply(result: LocalizationResult): TOdomClient {
  ...
  get T_odom_client(): TOdomClient | null {
    return this.current;
  }
