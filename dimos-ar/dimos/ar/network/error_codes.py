...
REGISTRATION_COMMIT_NO_CANDIDATE = BridgeError(
    ...
    fix="Wait for bridge confirmation or restart the robot and bridge (./launcher/scripts/start.sh).",
)

REGISTRATION_FAILED = BridgeError(
    code=500,
    description="AprilTag or manual registration failed on the bridge.",
    fix="Retry registration; restart the robot and bridge (./launcher/scripts/start.sh) if it persists.",
)
...
REGISTRATION_SESSION_UNAVAILABLE = BridgeError(
    ...
    fix="Restart the robot and bridge (./launcher/scripts/start.sh), then reconnect Spectacles.",
)
...
NAV_GOAL_STALLED = BridgeError(
    ...
    fix="Reconnect Spectacles to the bridge or restart the robot and bridge (./launcher/scripts/start.sh).",
)
...
CONTROL_RPC_TIMEOUT = BridgeError(
    ...
    fix="Use the robot's hardware/controller stop path, then restart the robot and bridge (./launcher/scripts/start.sh).",
)
