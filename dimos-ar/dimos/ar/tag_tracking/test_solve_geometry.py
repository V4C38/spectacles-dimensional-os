...
def test_capture_max_distance_m_applies_margin() -> None:
    base = max_detection_distance_m(560.0, TAG_TOTAL_SIZE_M, 24.0)
    assert capture_max_distance_m(560.0, TAG_TOTAL_SIZE_M, 24.0) == pytest.approx(
...
def test_capture_max_distance_m_rejects_non_positive_margin() -> None:
    with pytest.raises(ValueError, match="positive margin"):
        capture_max_distance_m(560.0, TAG_TOTAL_SIZE_M, 24.0, margin=0.0)
